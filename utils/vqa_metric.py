...
class VQAEval:
    ...
    def _clean_up(self,ans):
        result = ans.replace("\n", " ")
        result = result.replace("\t", " ")
        result = result.strip()
        result = self.processPunctuation(ans)
        result = self.processDigitArticle(result)
        return result
    
    def evaluate_once(self,prediction,answers):
        for i  in range(len(answers)):
            # print(answers[i])
            answers[i] = self._clean_up(answers[i])
            # print("after process",answers[i])

        prediction = self._clean_up(prediction)   
        accs = []
        for gtAnsDatum in answers:
            
            otherGTAns = [
                item for item in answers if item != gtAnsDatum
            ]
            # print("===BEGIN====",gtAnsDatum,"=====",otherGTAns,"===END===",resAns,"===")
            matchingAns = [item for item in otherGTAns if prediction == item]
...
def compute_vqa_once(prediction,answers):
    vqaEval = VQAEval(None, None, n=2)
    return vqaEval.evaluate_once(prediction,answers)
