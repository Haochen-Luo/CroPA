...
class FlamingoLayer(nn.Module):
    def __init__(self, gated_cross_attn_layer, decoder_layer):
        ...
        self.decoder_layer = decoder_layer # This is from Llama model
        self.vis_x = None
...
class FlamingoLMMixin(nn.Module):
    ...
    def forward(self, *input, **kwargs):
        ...
        for layer in self.get_decoder().layers: # get_decoder() is from transformers.LlamaForCausalLM
            layer.condition_media_locations(media_locations)
            layer.condition_attend_previous(attend_previous)
        if "inputs_embeds" in kwargs:
            kwargs.pop("input_ids")
        mix_result = super().forward(
            *input, **kwargs
